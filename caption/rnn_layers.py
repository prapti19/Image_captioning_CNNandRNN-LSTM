...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    Why=np.dot(prev_h,Wh)+np.dot(x,Wx)+b
    cache=(x,prev_h, Wx, Wh, b,next_h,Why)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
    ...
    ##############################################################################
    (x,prev_h, Wx, Wh, b,next_h,Why)=cache
    dforward = (1 - np.tanh(Why)**2) * dnext_h
...
def rnn_backward(dh, cache):
    ...
    for i in reversed(range(T)):
        d_prevh+=dh[:,i,:]   #see NOTE imp
        dx[:,i,:], d_prevh, dWx_, dWh_, db_ =rnn_step_backward(d_prevh,cache[i])
...
def word_embedding_forward(x, W):
    ...
    ##############################################################################
    out = W[x, :]  #like i want 5th word ie index in the dictionary i.e. weight matrix
...
def lstm_backward(dh, cache):
    ...
    #############################################################################
    N,T,H=dh.shape
    _,Wx,_,_,_,_,_,_,_,_,_,_=cache[0]
    D=Wx.shape[0]
    ...
    dh0 = np.zeros((N, H))
    d_prevh = np.zeros((N, H))
    d_prevc = np.zeros((N, H))
    
    for i in reversed(range(T)):
        d_prevh+=dh[:,i,:]   #see NOTE imp
        dx[:,i,:], d_prevh,d_prevc, dWx_, dWh_, db_ =lstm_step_backward(d_prevh,d_prevc,cache[i])
